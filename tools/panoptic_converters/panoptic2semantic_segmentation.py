"""This script converts data in panoptic COCO format to semantic segmentation.
All segments with the same semantic class in one image are combined together.
...
from mmdet.core import MyJsonEncoder, get_traceback, rgb2id
...
def extract_semantic_single_core(proc_id, annotations_set,
    ...
    for working_idx, annotation in enumerate(annotations_set):
        ...
        try:
            pan_format = np.array(
                Image.open(
                    osp.join(segmentations_folder, annotation['file_name'])),
                dtype=np.uint32)
        except IOError:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--input_json_file', type=str, help='JSON file with panoptic data')
    parser.add_argument(
