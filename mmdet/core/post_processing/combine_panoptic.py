...
instance segmentation predictions. The procedure is described in section 7 of
the panoptic segmentation paper https://arxiv.org/pdf/1801.00868.pdf.
...
from ..utils import IdGenerator, MyJsonEncoder, id2rgb


def combine_to_panoptic(proc_id,
                        img_ids,
                        img_id2img,
                        inst_by_image,
                        sem_by_image,
    ...
    for idx, img_id in enumerate(img_ids):
        ...
        save_image_path = osp.join(segmentations_folder,
...
def combine_panoptic_predictions(semseg_json_file,
    ...
    for img_id in bbox_by_image.keys():
        ...
        bbox_by_image[img_id] = sorted(
            bbox_by_image[img_id], key=lambda el: -el['score'])
    ...
    for proc_id, img_ids in enumerate(img_ids_split):
        p = workers.apply_async(
            ...
            (proc_id, img_ids, img_id2img, inst_by_image, sem_by_image,
             bbox_by_image, segmentations_folder, bbox_overlap_thr,
             overlap_thr, stuff_area_limit, categories, using_bbox))
        processes.append(p)
    panoptic_json = []
    for p in processes:
...
if __name__ == '__main__':
    ...
    combine_panoptic_predictions(args.semseg_json_file, args.instseg_json_file,
                                 args.bbox_json_file, args.images_json_file,
                                 args.categories_json_file,
                                 args.segmentations_folder,
                                 args.panoptic_json_file, args.confidence_thr,
                                 args.bbox_overlap_thr, args.overlap_thr,
                                 args.stuff_area_limit, args.using_bbox)
